...
exports.init = function(config, outputFile) {
    ...
    var _events = {
        onBegin: function(){},
        motionCreated: function(motionId){},
    ...
    return {
        ...
        brain: {
            ...
            createMotion: function (config, body, uri) {
                ...
                if(motion.id && this.motionsIDs.indexOf(motion.id) == -1 && motion.id != '&nbsp;') {
                    ...
                    _events.motionCreated(motion.id);
                    if(this.debug) {
                        console.log(motion);
                    } else {
            ...
            exportJSON: function(outputFile) {
                var outputFile = outputFile || 'out/'+config.event+'-'+Math.floor(Date.now() / 1000)+'.json',
                    outputPath = outputFile.slice(0, outputFile.lastIndexOf('/')),
                    scope = this;
        ...
        run: function() {
            ...
            tarantula.on('done', function() {
                ...
                _events.onFinish(scope.brain.output);
                return;
            } );
